...
function list_vms() {
  VMS=$(prlctl list -a -i --json | /tmp/jq -r 'map(select((.OS == "win-10" or .OS == "win-11" or .OS=="ubuntu") and .State == "running") | {id:.ID, name: .Name, tools_state: .GuestTools.state})')
  echo "$VMS"
}
...
function get_list_of_updates() {
  ...
  prlctl exec $VM_ID pwsh -Command "Install-Module PSWindowsUpdate -Force -AllowClobber; Import-Module PSWindowsUpdate; Get-WindowsUpdate | ForEach-Object { \$_ | Select-Object -Property Title,KB,KBArticleIDs,Size,LastDeploymentChangeTime,Status } | ConvertTo-Json -Depth 5" >"$temp_output_file" 2>/dev/null
...
function install_windows_updates() {
  ...
  if [ -z "$TARGET_KB" ]; then
    if [ "$AUTO_REBOOT" = "true" ]; then
      RESULT=$(prlctl exec $VM_ID pwsh -Command "Install-Module PSWindowsUpdate -Force -AllowClobber; Import-Module PSWindowsUpdate; Install-WindowsUpdate -AcceptAll -AutoReboot | ConvertTo-Json -Depth 5")
    else
      RESULT=$(prlctl exec $VM_ID pwsh -Command "Install-Module PSWindowsUpdate -Force -AllowClobber; Import-Module PSWindowsUpdate; Install-WindowsUpdate -AcceptAll | ConvertTo-Json -Depth 5")
    fi
  else
    if [ "$AUTO_REBOOT" = "true" ]; then
      RESULT=$(prlctl exec $VM_ID pwsh -Command "Install-Module PSWindowsUpdate -Force -AllowClobber; Import-Module PSWindowsUpdate; Install-WindowsUpdate -KBArticleID $TARGET_KB -AcceptAll -AutoReboot | ConvertTo-Json -Depth 5")
    else
      RESULT=$(prlctl exec $VM_ID pwsh -Command "Install-Module PSWindowsUpdate -Force -AllowClobber; Import-Module PSWindowsUpdate; Install-WindowsUpdate -KBArticleID $TARGET_KB -AcceptAll | ConvertTo-Json -Depth 5")
    fi
  fi
  echo "COMPLETE"
}
...
function uninstall_windows_updates() {
  VM_ID=$1
  TARGET_KB=$2
  RESULT=$(prlctl exec $VM_ID pwsh -Command "Install-Module PSWindowsUpdate -Force -AllowClobber; Import-Module PSWindowsUpdate; Uninstall-WindowsUpdate -KBArticleID $TARGET_KB | ConvertTo-Json -Depth 5")
  echo "$RESULT"
}

function update_pd_tools() {
  VM_ID=$1
  RESULT=$(prlctl installtools $VM_ID)
  if [ $? -ne 0 ]; then
